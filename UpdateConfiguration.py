...
def main(argv):
  ...
  if hostName != "":
    t = ""
    grafanaInputFile = "../grafana-template.ini"
  ...
  # Set environment variable values in nginx Dockerfile:
  ngInputFile = "../nginx-Dockerfile-template"
  ngOutputFile = "./nginx/Dockerfile"
  with open(ngInputFile, "r", encoding = "utf-8") as reader:
    t = reader.read()
    
    for key, value in envVars:
      t = t.replace("VALUE_"+key, value)
    
    with open(ngOutputFile, "w", encoding = "utf-8") as writer:
      writer.write(t) 
